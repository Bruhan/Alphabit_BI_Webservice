...
public interface FinProjectRepository extends JpaRepository<FinProject, Integer> {
    @Query(value="SELECT * from ##plant##FINPROJECT ORDER BY CRAT DESC OFFSET ?2 ROWS FETCH NEXT ?3 ROW ONLY",nativeQuery = true)
    List<FinProject> getallproject(int page, int pcount);
    ...
    @Query(value="SELECT * from ##plant##FINPROJECT WHERE PROJECT_STATUS = ?2 ORDER BY CRAT DESC OFFSET ?3 ROWS FETCH NEXT ?4 ROW ONLY",nativeQuery = true)
    List<FinProject> getprojectbystatus(String status,int page, int pcount);
}
