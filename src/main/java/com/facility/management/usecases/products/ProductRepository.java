...
public interface ProductRepository extends JpaRepository<ItemMst,Long> {
    ...
    @Query(value="SELECT IT.ID AS id,IT.PLANT AS plant,ISNULL(IT.ITEM,'') AS item,ISNULL(IT.ITEMDESC,'') AS itemDesc," +
            ...
            "ISNULL((SELECT SUM(QTY) FROM ##plant##INVMST AS IV WHERE IV.ITEM = IT.ITEM GROUP BY IV.ITEM ),0)/(SELECT ISNULL(U.QPUOM,'1') " +
            "FROM ##plant##UOM AS U WHERE U.UOM = ISNULL(IT.SALESUOM,IT.PURCHASEUOM)) AS stockQty " +
    ...
    @Query(value="SELECT IT.ID AS id,IT.PLANT AS plant,ISNULL(IT.ITEM,'') AS item,ISNULL(IT.ITEMDESC,'') AS itemDesc," +
            ...
            "ISNULL(IT.PRD_BRAND_ID,'') AS brand,ISNULL(IT.ITEMTYPE,'') AS subCategory,ISNULL(IT.SALESUOM,'') AS salesUom," +
            "0 AS stockQty " +
            "FROM ##plant##ITEMMST AS IT WHERE IT.VENDNO = ?1",nativeQuery = true)
    List<productDaoPojo> getListOfProductsbysupplier(String suppliercode);
